    ...
    <section id="process" className="py-24 px-6 md:px-12 bg-foreground/[0.02]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute top-0 bottom-0 left-[28px] md:left-1/2 w-px bg-foreground/10 transform md:-translate-x-px"></div>
            ...
            {steps.map((step, index) => (
              ...
              >
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                  <div className={`flex-none z-10 w-14 h-14 rounded-full flex items-center justify-center glass shadow-lg ${index % 2 === 0 ? 'md:mr-auto' : 'md:order-last'}`}>
                    <span className="text-lg font-bold">{step.number}</span>
                  </div>
                  
                  <div className={`md:w-1/2 p-6 rounded-2xl glass-dark ${index % 2 === 0 ? 'md:mr-auto md:text-right' : 'md:ml-auto'}`}>
